...
const App = () => {
  return (
    <>
      ...
      <Layout title='Title 1' urlBg={LayoutImage} desc="First Layout">
      ...
      <Layout title='Title 2' colorBg='red' >
          <div className="flex">
            {
              POKEMONS.map(item => <PokemonCard key={item.id} name={item.name} img={item.img} id={item.id}
                                                type={item.type} values={item.values} abilities={item.abilities}
                                                stats={item.stats} base_experience={item.base_experience}
                                                height={item.height}/>)
            }
          </div>
      </Layout>
